        ...
        */
        ...
        function cadastrar(){
            /* utiliza o objeto de conexão criado no construtor e chama um metodo da classe conectar */
                $this->conn->ExecQuery("INSERT INTO sc_tp_usuario VALUES (null,'$this->nome','$this->codigo')");
                
        }
        /* Função para apagar o dado do responsável, esta não necessita do construtor pois esta vinculada diretamente ao id */
        function apagar($id)
        {
                $this->conn->ExecQuery("DELETE FROM sc_tp_usuario WHERE TU_cod = '".$id."'");
        }
        /*    */
        function editar($id)
            {
                $this->conn->ExecQuery("UPDATE sc_tp_usuario SET TU_cod = '$this->codigo',TU_nome='$this->nome' WHERE TU_cod = $id");			
            }
        function buscar($nome = null){
                if($nome == null)
                {
                    $sql = "SELECT * FROM `sc_tp_usuario`";
                }
                else   
                {
                         $sql = "SELECT * FROM sc_tp_usuario WHERE TU_nome ='".$nome."';";
                }
                ...
                /* Enquanrto o resultado da busca não chegar no ultimo dado registrado faça */
                    while ($campos = $query->fetch_assoc()) 
                    {
                        /* Impirimirá cada coluna de cada dado no banco */
                            echo '<tr>';
                            echo '  <td>'.$campos["TU_cod"].'</td><br/>';
                            echo '  <td>'.$campos["TU_nome"].'</td>';
                            //Cria uma variavel Superglobal enviado ao cabeçalho da pagina o valor do campo */
                            echo '  <td><a href="'.$_SERVER["PHP_SELF"].'?TU_nome='.$campos["TU_nome"].'">Editar</a></td>';
                            echo '  <td><a href="'.$_SERVER["PHP_SELF"].'?TU_nome='.$campos["TU_nome"].'&del=true">Excluir</a></td>';
                            echo '</tr>';
                    }
