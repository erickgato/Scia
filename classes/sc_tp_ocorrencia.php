        ...
        */
        ...
        function cadastrar(){
            /* utiliza o objeto de conexão criado no construtor e chama um metodo da classe conectar */
                $this->conn->ExecQuery("INSERT INTO sc_tp_ocorrencia
                 VALUES (null,'$this->nome','$this->codigo')");
                
        }
        /* Função para apagar o dado do responsável, esta não necessita do construtor pois esta vinculada diretamente ao id */
        function apagar($id)
        {
                $this->conn->ExecQuery("DELETE FROM sc_tp_ocorrencia
                 WHERE TO_cod = '".$id."'");
        }
        /*    */
        function editar($id)
            {
                $this->conn->ExecQuery("UPDATE sc_tp_ocorrencia
                 SET TO_cod = '$this->codigo',TO_nome='$this->nome' WHERE TO_cod = $id");			
            }
        function buscar($nome = null){
                if($nome == null)
                {
                    $sql = "SELECT * FROM `sc_tp_ocorrencia
                    `";
                }
                else   
                {
                ...
                /* Enquanrto o resultado da busca não chegar no ultimo dado registrado faça */
                    while ($campos = $query->fetch_assoc()) 
                    {
                        /* Impirimirá cada coluna de cada dado no banco */
                            echo '<tr>';
                            echo '  <td>'.$campos["TO_cod"].'</td><br/>';
                            echo '  <td>'.$campos["TO_nome"].'</td>';
                            //Cria uma variavel Superglobal enviado ao cabeçalho da pagina o valor do campo */
                            echo '  <td><a href="'.$_SERVER["PHP_SELF"].'?TO_nome='.$campos["TO_nome"].'">Editar</a></td>';
                            echo '  <td><a href="'.$_SERVER["PHP_SELF"].'?TO_nome='.$campos["TO_nome"].'&del=true">Excluir</a></td>';
                            echo '</tr>';
                    }
